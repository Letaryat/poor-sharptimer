...
public partial class SharpTimer
{
        ...
        foreach (var kvp in playerTimers)
        {
            ...
            foreach (var prop in typeof(PlayerTimerInfo).GetProperties())
            {
                ...
                if (value is Dictionary<int, int> intIntDictionary)
                {
                    ...
                    foreach (var entry in intIntDictionary) SharpTimerConPrint($"      {entry.Key}: {entry.Value}");
                }
                else if (value is Dictionary<int, string> intStringDictionary)
                {
                    SharpTimerConPrint($"    {prop.Name}:");
                    foreach (var entry in intStringDictionary) SharpTimerConPrint($"      {entry.Key}: {entry.Value}");
                }
            }
    ...
    public void ReplaySelfCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () => await ReplayHandler(player, playerSlot, "self", steamID, playerName, 0,
    ...
    public void ReplaySRCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () =>
            await ReplayHandler(player, playerSlot, "1", "69", "unknown", 0, playerTimers[playerSlot].currentStyle));
    }

    [ConsoleCommand("css_replaytop", "Replay a top 10 server map record")]
    [CommandHelper(1, "[1-10]", CommandUsage.CLIENT_ONLY)]
    public void ReplayTop10SRCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () =>
            await ReplayHandler(player, playerSlot, arg, "69", "unknown", 0, playerTimers[playerSlot].currentStyle));
    }
    ...
    public void ReplayTop10WRCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () => await ReplayHandler(player, playerSlot, arg, "69", "unknown", 0,
            playerTimers[playerSlot].currentStyle, true));
    }
    ...
    [CommandHelper(1, "[1-10] [bonus stage]", CommandUsage.CLIENT_ONLY)]
    public void ReplayBonusCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () => await ReplayHandler(player, playerSlot, arg, "69", "unknown", short.Parse(arg2),
            playerTimers[playerSlot].currentStyle));
    }

    [ConsoleCommand("css_replaybpb", "Replay your bonus pb")]
    [ConsoleCommand("css_replaybonuspb", "Replay your bonus pb")]
    [CommandHelper(1, "[bonus stage]", CommandUsage.CLIENT_ONLY)]
    public void ReplayBonusPBCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        var arg = command.ArgByIndex(1);
        int bonusX = short.Parse(arg);

        _ = Task.Run(async () => await ReplayHandler(player, playerSlot, "self", steamID, playerName, bonusX,
            playerTimers[playerSlot].currentStyle));
    }

    public async Task ReplayHandler(CCSPlayerController player, int playerSlot, string arg = "1",
        string pbSteamID = "69", string playerName = "unknown", int bonusX = 0, int style = 0, bool wr = false)
    {
        var self = false;
        ...
        if (wr)
            ...
            playerTimers[playerSlot].ReplayHUDString =
                !self ? $"{srPlayerName} | {srTime}" : $"{playerName} | {playerTimers[playerSlot].CachedPB}";
        ...
        if (stageTriggers.Count != 0)
            playerTimers[playerSlot].StageTimes!.Clear(); //remove previous stage times if the map has stages
        if (stageTriggers.Count != 0)
            playerTimers[playerSlot].StageVelos!.Clear(); //remove previous stage times if the map has stages
    ...
    public void SpecCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        else if (player.Team == CsTeam.CounterTerrorist || player.Team == CsTeam.Terrorist ||
    ...
    public void HUDSwitchCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (enableDb) _ = Task.Run(async () => await SetPlayerStats(player, steamID, playerName, playerSlot));
    }
    ...
    public void SoundsSwitchCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        playerTimers[playerSlot].SoundsEnabled = !playerTimers[playerSlot].SoundsEnabled;
    ...
    [CommandHelper(1, "[fov]", CommandUsage.CLIENT_ONLY)]
    public void FovCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (!int.TryParse(command.GetArg(1), out var desiredFov)) return;
    ...
    public void PrintTopRecords(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () => await PrintTopRecordsHandler(player, playerName, 0,
            string.IsNullOrEmpty(mapName) ? "" : mapName, playerTimers[player.Slot].currentStyle));
    }
    ...
    public void PrintTopBonusRecords(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (!int.TryParse(command.ArgString, out var bonusX))
        {
    ...
    public async Task PrintTopRecordsHandler(CCSPlayerController? player, string playerName, int bonusX = 0,
        string mapName = "", int style = 0)
    {
        ...
        if (bonusX != 0)
            printStatements =
            [
                $" {Localizer["prefix"]} {Localizer["top10_records_bonus", GetNamedStyle(style), bonusX, currentMapNamee]}"
            ];
        else
            printStatements =
                [$" {Localizer["prefix"]} {Localizer["top10_records", GetNamedStyle(style), currentMapNamee]}"];

        var rank = 1;

        foreach (var kvp in sortedRecords.Take(10))
        {
            var _playerName = kvp.Value.PlayerName!;
            var timerTicks = kvp.Value.TimerTicks;

            var showReplays = false;
            if (enableReplays) showReplays = await CheckSRReplay(kvp.Value.SteamID!, bonusX);

            var replayIndicator = enableReplays ? showReplays ? $"{ChatColors.Red}◉" : "" : "";

            printStatements.Add(
                $" {Localizer["prefix"]} {Localizer["records_map", rank, _playerName, replayIndicator, FormatTime(timerTicks)]}");
            rank++;
        }
    ...
    public void RankCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        _ = Task.Run(async () => await RankCommandHandler(player, steamID, playerSlot, playerName, false,
            playerTimers[playerSlot].currentStyle));
    }

    public async Task RankCommandHandler(CCSPlayerController? player, string steamId, int playerSlot, string playerName,
        bool sendRankToHUD = false, int style = 0)
    {
        try
        {
            if (!IsAllowedClient(player))
            ...
            var useGlobalRanks = enableDb && globalRanksEnabled;

            ranking = useGlobalRanks
                ? await GetPlayerServerPlacement(player, steamId, playerName)
                : await GetPlayerMapPlacementWithTotal(player, steamId, playerName, false, false, 0, style);
            rankIcon = useGlobalRanks
                ? await GetPlayerServerPlacement(player, steamId, playerName, true)
                : await GetPlayerMapPlacementWithTotal(player, steamId, playerName, true, false, 0, style);
            ...
            foreach (var bonusRespawnPose in bonusRespawnPoses)
            {
                ...
                var bonusPbTicks = enableDb
                    ? await GetPreviousPlayerRecordFromDatabase(steamId, currentMapName!, playerName, bonusNumber,
                        style)
                    : await GetPreviousPlayerRecord(steamId, bonusNumber);

                /// Skip this bonus since the player doesn't have a saved time
                if (bonusPbTicks <= 0) continue;

                var bonusPlacement =
                    await GetPlayerMapPlacementWithTotal(player, steamId, playerName, false, true, bonusNumber, style);
            ...
            if (useGlobalRanks)
            {
                ...
                serverPlacement = await GetPlayerServerPlacement(player, steamId, playerName, false, true);
            }

            var pbTicks = enableDb
                ? await GetPreviousPlayerRecordFromDatabase(steamId, currentMapName!, playerName, 0, style)
                : await GetPreviousPlayerRecord(steamId);
            ...
            if (!sendRankToHUD)
                Server.NextFrame(() =>
                {
                    ...
                    if (pbTicks != 0)
                        PrintToChat(player,
                            Localizer["current_pb", currentMapName!, FormatTime(pbTicks), mapPlacement]);
                    ...
                    if (playerTimers[playerSlot].CachedBonusInfo.Any())
                        foreach (var bonusPb in playerTimers[playerSlot].CachedBonusInfo.OrderBy(x => x.Key))
                            PrintToChat(player,
                                $"{Localizer["current_bonus_pb", bonusPb.Key!, FormatTime(bonusPb.Value.PbTicks), bonusPb.Value.Placement!]}");
                });
        }
        catch (Exception ex)
        {
    ...
    public void RespawnBonusPlayer(CCSPlayerController? player, CommandInfo command)
    {
        try
        {
            ...
            if (command.ArgString == null || command.ArgString == "")
            {
                ...
                {
                    if (bonusRespawnAngs.TryGetValue(1, out var bonusAng) && bonusAng != null)
                        player.PlayerPawn.Value!.Teleport(bonusRespawnPoses[1]!, bonusRespawnAngs[1]!,
                            new Vector(0, 0, 0));
                    else
                        player.PlayerPawn.Value!.Teleport(bonusRespawnPoses[1]!,
                            new QAngle(player.PlayerPawn.Value.EyeAngles.X, player.PlayerPawn.Value.EyeAngles.Y,
                                player.PlayerPawn.Value.EyeAngles.Z) ?? new QAngle(0, 0, 0), new Vector(0, 0, 0));
            ...
            {
                if (bonusRespawnAngs.TryGetValue(bonusX, out var bonusAng) && bonusAng != null)
                    player.PlayerPawn.Value!.Teleport(bonusRespawnPoses[bonusX]!, bonusRespawnAngs[bonusX]!,
                        new Vector(0, 0, 0));
                else
                    player.PlayerPawn.Value!.Teleport(bonusRespawnPoses[bonusX]!,
                        new QAngle(player.PlayerPawn.Value.EyeAngles.X, player.PlayerPawn.Value.EyeAngles.Y,
                            player.PlayerPawn.Value.EyeAngles.Z) ?? new QAngle(0, 0, 0), new Vector(0, 0, 0));
    ...
    public void SetRespawnCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        var currentPosition = player.Pawn.Value!.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0);
        var currentRotation = player.PlayerPawn.Value!.EyeAngles ?? new QAngle(0, 0, 0);

        if (useTriggers)
        {
            if (IsVectorInsideBox(currentPosition + new Vector(0, 0, 10), currentMapStartTriggerMaxs!,
                    currentMapStartTriggerMins!))
            {
    ...
    public void TPtoStagePlayer(CCSPlayerController? player, CommandInfo command)
    {
        try
        {
            ...
            if (!int.TryParse(command.ArgString, out var stageX))
            {
            ...
            if (stageTriggerPoses.TryGetValue(stageX, out var stagePos) && stagePos != null)
            {
                player.PlayerPawn.Value!.Teleport(stagePos,
                    stageTriggerAngs[stageX] ?? player.PlayerPawn.Value.EyeAngles, new Vector(0, 0, 0));
    ...
    public void RespawnPlayerCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (playerTimers[player.Slot].IsReplaying)
        {
            ...
            if (stageTriggers.Count != 0)
                playerTimers[player.Slot].StageTimes!.Clear(); //remove previous stage times if the map has stages
            if (stageTriggers.Count != 0)
                playerTimers[player.Slot].StageVelos!.Clear(); //remove previous stage times if the map has stages
            RespawnPlayer(player);
        }
        else
        {
    ...
    public void StyleCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            for (var i = 0; i < 11; i++) //runs 11 times for the 11 styles (i=0-10)
                PrintToChat(player, Localizer["styles_list", i, GetNamedStyle(i)]);
            PrintToChat(player, Localizer["style_example"]);
            return;
        }

        if (int.TryParse(command.GetArg(1), out var desiredStyleInt))
        {
        ...
        {
            var styleLowerCase = desiredStyle.ToLower();
    ...
    public void RespawnPlayer(CCSPlayerController? player, bool toEnd = false)
    {
        try
        {
            ...
            if (stageTriggerCount != 0 ||
                cpTriggerCount != 0) //remove previous stage times and checkpoints if the map has stages or checkpoints
            {
            ...
            if (toEnd == false)
            {
                if (currentRespawnPos != null && playerTimers[player.Slot].SetRespawnPos == null)
                {
                    if (currentRespawnAng != null)
                        ...
                        player.PlayerPawn.Value!.Teleport(currentRespawnPos,
                            player.PlayerPawn.Value.EyeAngles ?? new QAngle(0, 0, 0), new Vector(0, 0, 0));
                    SharpTimerDebug($"{player.PlayerName} css_r to {currentRespawnPos}");
                }
                else
                {
                    if (playerTimers[player.Slot].SetRespawnPos != null &&
                        playerTimers[player.Slot].SetRespawnAng != null)
                        player.PlayerPawn.Value!.Teleport(ParseVector(playerTimers[player.Slot].SetRespawnPos!),
                            ParseQAngle(playerTimers[player.Slot].SetRespawnAng!), new Vector(0, 0, 0));
                    else
            ...
            else
            {
                if (currentEndPos != null)
                    player.PlayerPawn.Value!.Teleport(currentEndPos,
                        player.PlayerPawn.Value.EyeAngles ?? new QAngle(0, 0, 0), new Vector(0, 0, 0));
                else
    ...
    public void RestartCurrentStageCmd(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (!playerTimers.TryGetValue(player.Slot, out var playerTimer) || playerTimer.CurrentMapStage == 0)
        {
        ...
        var currStage = playerTimer.CurrentMapStage;

        try
        {
            playerTimers[player.Slot].TicksSinceLastCmd = 0;

            if (stageTriggerPoses.TryGetValue(currStage, out var stagePos) && stagePos != null)
            {
                if (jumpStatsEnabled) InvalidateJS(player.Slot);
                player.PlayerPawn.Value!.Teleport(stagePos,
                    stageTriggerAngs[currStage] ?? player.PlayerPawn.Value.EyeAngles, new Vector(0, 0, 0));
    ...
    */
    ...
    [CommandHelper(1, "[name]", CommandUsage.CLIENT_ONLY)]
        ...
        if (foundPlayer != null && playerTimers[player.Slot].IsTimerBlocked)
        {
            ...
            if (player != null && IsAllowedPlayer(foundPlayer) && playerTimers[player.Slot].IsTimerBlocked)
            {
                ...
                player.PlayerPawn.Value!.Teleport(
                    foundPlayer.Pawn.Value!.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0),
                    foundPlayer.PlayerPawn.Value!.EyeAngles ?? new QAngle(0, 0, 0), new Vector(0, 0, 0));
                SharpTimerDebug(
                    $"{player.PlayerName} css_goto to {foundPlayer.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0)}");
            }
        }
        else
        {
    ...
    public void SetPlayerCP(CCSPlayerController? player, CommandInfo command)
    {
        if (((PlayerFlags)player!.Pawn.Value!.Flags & PlayerFlags.FL_ONGROUND) != PlayerFlags.FL_ONGROUND &&
            removeCpRestrictEnabled == false)
        {
            PrintToChat(player,
                Localizer["cant_use_checkpoint_in_air", currentMapName!.Contains("surf_") ? "loc" : "checkpoint"]);
        ...
        // Get the player's current position and rotation
        var currentPosition = player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0);
        var currentSpeed = player.PlayerPawn.Value!.AbsVelocity ?? new Vector(0, 0, 0);
        var currentRotation = player.PlayerPawn.Value.EyeAngles ?? new QAngle(0, 0, 0);

        // Convert position and rotation to strings
        var positionString = $"{currentPosition.X} {currentPosition.Y} {currentPosition.Z}";
        var rotationString = $"{currentRotation.X} {currentRotation.Y} {currentRotation.Z}";
        var speedString = $"{currentSpeed.X} {currentSpeed.Y} {currentSpeed.Z}";

        // Add the current position and rotation strings to the player's checkpoint list
        if (!playerCheckpoints.ContainsKey(player.Slot)) playerCheckpoints[player.Slot] = [];
        ...
        var checkpointCount = playerCheckpoints[player.Slot].Count;
        playerTimers[player.Slot].CheckpointIndex = checkpointCount - 1;

        // Print the chat message with the checkpoint count
        PrintToChat(player,
            Localizer["checkpoint_set", currentMapName!.Contains("surf_") ? "loc" : "checkpoint", checkpointCount]);
        PlaySound(player, cpSound);
        SharpTimerDebug(
            $"{player.PlayerName} css_cp to {checkpointCount} {positionString} {rotationString} {speedString}");
    }
    ...
    public void TpPlayerCP(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            PrintToChat(player,
                Localizer["no_checkpoint_set", currentMapName!.Contains("surf_") ? "loc" : "checkpoint"]);
            PlaySound(player, cpSoundError);
            return;
        }
        ...
        var lastCheckpoint = playerCheckpoints[player.Slot][playerTimers[player.Slot].CheckpointIndex];

        // Convert position and rotation strings to Vector and QAngle
        var position = ParseVector(lastCheckpoint.PositionString ?? "0 0 0");
        var rotation = ParseQAngle(lastCheckpoint.RotationString ?? "0 0 0");
        var speed = ParseVector(lastCheckpoint.SpeedString ?? "0 0 0");

        // Teleport the player to the most recent checkpoint, including the saved rotation
        if (removeCpRestrictEnabled)
        ...
        PrintToChat(player,
            Localizer["used_recent_checkpoint", currentMapName!.Contains("surf_") ? "loc" : "checkpoint"]);
    ...
    public void TpPreviousCP(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (!playerCheckpoints.TryGetValue(player.Slot, out List<PlayerCheckpoint>? checkpoints) ||
            checkpoints.Count == 0)
        {
            PrintToChat(player,
                Localizer["no_checkpoint_set", currentMapName!.Contains("surf_") ? "loc" : "checkpoint"]);
        ...
        var index = playerTimers.TryGetValue(player.Slot, out var timer) ? timer.CheckpointIndex : 0;
        ...
        else
        {
            ...
            var previousCheckpoint = checkpoints[index];
            ...
            PlaySound(player, tpSound);
            PrintToChat(player,
                Localizer["used_previous_checkpoint", currentMapName!.Contains("surf_") ? "loc" : "checkpoint"]);
    ...
    public void TpNextCP(CCSPlayerController? player, CommandInfo command)
    {
        ...
        else
        {
            ...
            // Convert position and rotation strings to Vector and QAngle
            var position = ParseVector(nextCheckpoint.PositionString ?? "0 0 0");
            var rotation = ParseQAngle(nextCheckpoint.RotationString ?? "0 0 0");
            var speed = ParseVector(nextCheckpoint.SpeedString ?? "0 0 0");
