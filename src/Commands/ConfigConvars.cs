...
public partial class SharpTimer
{
    ...
    [ConsoleCommand("sharptimer_autoset_mapinfo_hostname_enabled",
        "Whether Map Name and Map Tier (if available) should be put into the hostname or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerHostnameConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        autosetHostname = bool.TryParse(args, out var autosetHostnameValue)
            ? autosetHostnameValue
            : args != "0" && autosetHostname;
    }

    [ConsoleCommand("sharptimer_custom_map_cfgs_enabled",
        "Whether Custom Map .cfg files should be executed for the corresponding maps (found in cfg/SharpTimer/MapData/MapExecs/kz_example.cfg). Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCustomMapExecConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        execCustomMapCFG = bool.TryParse(args, out var execCustomMapCFGValue)
            ? execCustomMapCFGValue
            : args != "0" && execCustomMapCFG;
    }

    [ConsoleCommand("sharptimer_display_rank_tags_chat",
        "Whether the plugin should display rank tags infront of players names in chat or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerChatRankTagsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        displayChatTags = bool.TryParse(args, out var displayChatTagsValue)
            ? displayChatTagsValue
            : args != "0" && displayChatTags;
    }

    [ConsoleCommand("sharptimer_display_rank_tags_scoreboard",
        "Whether the plugin should display rank tags infront of players names in scoreboard or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerScoreboardRankTagsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        displayScoreboardTags = bool.TryParse(args, out var displayScoreboardTagsValue)
            ? displayScoreboardTagsValue
            : args != "0" && displayScoreboardTags;
    }

    [ConsoleCommand("sharptimer_global_rank_points_enabled",
        "Whether the plugin should reward players with global points for completing maps. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        globalRanksEnabled = bool.TryParse(args, out var globalRanksEnabledValue)
            ? globalRanksEnabledValue
            : args != "0" && globalRanksEnabled;
    }

    [ConsoleCommand("sharptimer_global_rank_min_points_threshold",
        "Players with Points below this amount will be treated as Unranked. Default value: 1")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksMinPointsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var minPoints) && minPoints > 0)
        {
    ...
    [ConsoleCommand("sharptimer_global_rank_bonus_points_multiplier",
        "Multiplier for bonus course completion points. Default value: 0.5")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksBonusPointsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (double.TryParse(args, out var bonusMultiplier) && bonusMultiplier > 0)
        {
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t1",
        "Baseline points awarded for completing a T1 map. Default value: 25")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksBaselineT1Convar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var baseline) && baseline > 0)
        {
            baselineT1 = baseline;
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t2",
        "Baseline points awarded for completing a T2 map. Default value: 50")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t3",
        "Baseline points awarded for completing a T3 map. Default value: 100")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t4",
        "Baseline points awarded for completing a T4 map. Default value: 200")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t5",
        "Baseline points awarded for completing a T5 map. Default value: 400")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t6",
        "Baseline points awarded for completing a T6 map. Default value: 600")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t7",
        "Baseline points awarded for completing a T7 map. Default value: 800")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_baseline_t8",
        "Baseline points awarded for completing a T8 map. Default value: 1000")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_max_record_points",
        "Baseline max points for records, multiplied by tiernum. Default value: 250")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_max_completions",
        "Maximum completions allowed to receive points (0 = infinite). Default value: 0")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksMaxCompletionsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var completions) && completions >= 0)
        {
    ...
    public void SharpTimerGlobalRanksTop101Convar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (double.TryParse(args, out var multiplier) && multiplier >= 0)
        {
            top10_1 = multiplier;
    ...
    [ConsoleCommand("sharptimer_global_rank_group1_percentile",
        "The top % of record holders in group #1. Default value: 3.125")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalRanksGroup1Convar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (double.TryParse(args, out var percentile) && percentile >= 0)
        {
            group1 = percentile;
    ...
    [ConsoleCommand("sharptimer_global_rank_group2_percentile",
        "The top % of record holders in group #2. Default value: 6.25")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_group3_percentile",
        "The top % of record holders in group #3. Default value: 12.5")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_group4_percentile",
        "The top % of record holders in group #4. Default value: 25")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_global_rank_group5_percentile",
        "The top % of record holders in group #5. Default value: 50")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_replays_enabled",
        "Whether replays should be enabled or not. This option might be performance taxing and use more ram & cpu. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerReplayConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        enableReplays = bool.TryParse(args, out var enableReplaysValue)
            ? enableReplaysValue
            : args != "0" && enableReplays;
    }

    [ConsoleCommand("sharptimer_replay_max_length",
        "The maximum length for a Replay to be saved in seconds. Anything longer will be discarded Default value: 300")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerReplayMaxLengthConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var mxLength) && mxLength > 0)
        {
            maxReplayFrames = mxLength * 64;
    ...
    [ConsoleCommand("sharptimer_replay_bot_enabled",
        "Whether a looping Server Record bot should be spawned in or not (requires navmesh fix). Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerReplayBotConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        enableSRreplayBot = bool.TryParse(args, out var enableSRreplayBotValue)
            ? enableSRreplayBotValue
            : args != "0" && enableSRreplayBot;
    }

    [ConsoleCommand("sharptimer_replay_bot_name",
        "What the name of the Replay Record bot should be. Default value: SERVER RECORD REPLAY")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    public void SharpTimerJumpStatsConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        jumpStatsEnabled = bool.TryParse(args, out var jumpStatsEnabledValue)
            ? jumpStatsEnabledValue
            : args != "0" && jumpStatsEnabled;
    }

    [ConsoleCommand("sharptimer_jumpstats_min_distance",
        "Defines the minimum distance for a jumpstat to be printed to chat. Default value: 175.0")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerJumpStatsMinDistConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (float.TryParse(args, out var dist) && dist > 0)
        {
    ...
    [ConsoleCommand("sharptimer_jumpstats_max_vert",
        "Defines the max vertical distance for a jumpstat to not be printed to chat. Default value: 32.0")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_jumpstats_movement_unlocker_cap",
        "Intended for taming movement unlocker, caps speed on the second tick of a player being on the ground. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerJumpStatsUnlockerCapConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        movementUnlockerCapEnabled = bool.TryParse(args, out var movementUnlockerCapEnabledValue)
            ? movementUnlockerCapEnabledValue
            : args != "0" && movementUnlockerCapEnabled;
    }

    [ConsoleCommand("sharptimer_jumpstats_movement_unlocker_cap_value",
        "Speed cap value which will kick in on the second tick of the player being on the ground. Default value: 250.0")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerJumpStatsUnlockerCapValueConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (float.TryParse(args, out var value) && value > 0)
        {
            movementUnlockerCapValue = value;
            if (movementUnlockerCapEnabled)
                SharpTimerConPrint($"SharpTimer JumpStats Movement Unlocker cap value set to {value} units.");
        }
        else
        {
    ...
    [ConsoleCommand("sharptimer_global_cache_interval",
        "Refresh interval of record and point caching. Default value: 120")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerGlobalCacheConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var value) && value > 0)
        {
    ...
    [ConsoleCommand("sharptimer_kill_pointservercommand_entities",
        "If True the plugin will kill all point_servercommand ents (necessary to make xplay maps usable due to them being bad ports). Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerPointServerCommandConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        killServerCommands = bool.TryParse(args, out var killServerCommandsValue)
            ? killServerCommandsValue
            : args != "0" && killServerCommands;
    }

    [ConsoleCommand("sharptimer_enable_anticheat",
        "If enabled, suspicious players will be flagged. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerAnticheatConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        useAnticheat = bool.TryParse(args, out var value) ? value : args != "0" && useAnticheat;
    }

    [ConsoleCommand("sharptimer_enable_afk_hibernation",
        "If True the plugin will reduce functionality to afk players to improve performance. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerAFKHibernationConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        afkHibernation = bool.TryParse(args, out var afkValue) ? afkValue : args != "0" && afkHibernation;
    }

    [ConsoleCommand("sharptimer_enable_afk_warning",
        "If true the plugin will warn the player who is about to be moved to afk. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerAFKWarningConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        afkWarning = bool.TryParse(args, out var afkValue) ? afkValue : args != "0" && afkWarning;
    }

    [ConsoleCommand("sharptimer_afk_seconds",
        "Amount of seconds a player is allowed to afk before being hibernated. Default value: 60")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerAFKSecondsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var secs) && secs >= 0)
        {
            afkSeconds = secs;
    ...
    public void SharpTimerHUDUpdatesConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (int.TryParse(args, out var tickrate) && tickrate >= 0 && tickrate <= 64)
        {
    ...
    [ConsoleCommand("sharptimer_enable_timer_hud",
        "If Timer Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        hudOverlayEnabled = bool.TryParse(args, out var hudOverlayEnabledValue)
            ? hudOverlayEnabledValue
            : args != "0" && hudOverlayEnabled;
    }
    ...
    public void SharpTimerPointKeysHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        keysOverlayEnabled = bool.TryParse(args, out var keysOverlayEnabledValue)
            ? keysOverlayEnabledValue
            : args != "0" && keysOverlayEnabled;
    }

    [ConsoleCommand("sharptimer_enable_rankicons_hud",
        "If Rank Icons Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerPointRankIconsHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        RankIconsEnabled = bool.TryParse(args, out var rankIconsEnabledValue)
            ? rankIconsEnabledValue
            : args != "0" && RankIconsEnabled;
    }

    [ConsoleCommand("sharptimer_enable_velocity_hud",
        "If Speed Velocity Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerVelocityHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        VelocityHudEnabled = bool.TryParse(args, out var VelocityHudValue)
            ? VelocityHudValue
            : args != "0" && VelocityHudEnabled;
    }

    [ConsoleCommand("sharptimer_enable_strafesync_hud",
        "If Stafe Sync % Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerStrafeHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        StrafeHudEnabled = bool.TryParse(args, out var StrafeHudValue)
            ? StrafeHudValue
            : args != "0" && StrafeHudEnabled;
    }

    [ConsoleCommand("sharptimer_enable_map_tier_hud",
        "If Map Tier Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMapTierHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        MapTierHudEnabled = bool.TryParse(args, out var MapTierHudValue)
            ? MapTierHudValue
            : args != "0" && MapTierHudEnabled;
    }

    [ConsoleCommand("sharptimer_enable_map_type_hud",
        "If Map Type Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMapTypeHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        MapTypeHudEnabled = bool.TryParse(args, out var MapTypeHudValue)
            ? MapTypeHudValue
            : args != "0" && MapTypeHudEnabled;
    }

    [ConsoleCommand("sharptimer_enable_map_name_hud",
        "If Map Name Hud should be globally enabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMapNameHUDConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        MapNameHudEnabled = bool.TryParse(args, out var MapNameHudValue)
            ? MapNameHudValue
            : args != "0" && MapNameHudEnabled;
    }
    ...
    public void SharpTimerConPrintConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        enableDebug = bool.TryParse(args, out var enableDebugValue) ? enableDebugValue : args != "0" && enableDebug;
    }

    [ConsoleCommand("sharptimer_remote_data_disable",
        "Disable fetching map tier and type from github. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerRemoteDataConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        disableRemoteData = bool.TryParse(args, out var value) ? value : args != "0" && disableRemoteData;
    }
    ...
    [ConsoleCommand("sharptimer_enable_checkpoint_verification",
        "Enable or disable checkpoint verification system. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCheckpointVerificationConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        useCheckpointVerification = bool.TryParse(args, out var CheckpointVerificationValue)
            ? CheckpointVerificationValue
            : args != "0" && useCheckpointVerification;
    }
    ...
    public void SharpTimer2dSpeedConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        use2DSpeed = bool.TryParse(args, out var use2DSpeedValue) ? use2DSpeedValue : args != "0" && use2DSpeed;
    }
    ...
    public void SharpTimerBeamColorsConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        beamColorOverride = bool.TryParse(args, out var beamColorOverrideValue)
            ? beamColorOverrideValue
            : args != "0" && beamColorOverride;
    }

    [ConsoleCommand("sharptimer_start_beam_color",
        "Start beam color, Requires sharptimer_override_beam_colors_enabled true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_end_beam_color",
        "Start beam color, Requires sharptimer_override_beam_colors_enabled true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_mysql_enabled",
        "Whether player times should be put into a mysql database by default or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMySQLConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        useMySQL = bool.TryParse(args, out var useMySQLValue) ? useMySQLValue : args != "0" && useMySQL;
    }

    [ConsoleCommand("sharptimer_postgres_enabled",
        "Whether player times should be put into a postgres database by default or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerPostgresConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        usePostgres = bool.TryParse(args, out var usePostgresValue) ? usePostgresValue : args != "0" && usePostgres;
    }

    [ConsoleCommand("sharptimer_discordwebhook_enabled",
        "Whether player PBs or SRs should be printed into a discord channel or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerDiscordWebhookConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        discordWebhookEnabled = bool.TryParse(args, out var discordWebhookEnabledValue)
            ? discordWebhookEnabledValue
            : args != "0" && discordWebhookEnabled;
    }
    ...
    public void SharpTimerDiscordWebhookSRConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        discordWebhookPrintSR = bool.TryParse(args, out var discordWebhookPrintSRValue)
            ? discordWebhookPrintSRValue
            : args != "0" && discordWebhookPrintSR;
    }
    ...
    public void SharpTimerDiscordWebhookPBConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        discordWebhookPrintPB = bool.TryParse(args, out var discordWebhookPrintPBValue)
            ? discordWebhookPrintPBValue
            : args != "0" && discordWebhookPrintPB;
    }

    [ConsoleCommand("sharptimer_force_disable_json",
        "Whether player times should NOT be saved to JSON. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerIgnoreJSONConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        ignoreJSON = bool.TryParse(args, out var ignoreJSONValue) ? ignoreJSONValue : args != "0" && ignoreJSON;
    }

    [ConsoleCommand("sharptimer_command_spam_cooldown",
        "Defines the time between commands can be called. Default value: 1")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCmdCooldownConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (float.TryParse(args, out var cooldown) && cooldown > 0)
        {
    ...
    [ConsoleCommand("sharptimer_max_bhop_block_time",
        "Defines the time the player is allowed to stand on a Bhop block (if the map supports it). Default value: 1")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerBhopBlockConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (float.TryParse(args, out var time) && time > 0)
        {
    ...
    public void SharpTimerRespawnConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        respawnEnabled = bool.TryParse(args, out var respawnEnabledValue)
            ? respawnEnabledValue
            : args != "0" && respawnEnabled;
    }
    ...
    public void SharpTimerRespawnEndConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        respawnEndEnabled = bool.TryParse(args, out var respawnEndEnabledValue)
            ? respawnEndEnabledValue
            : args != "0" && respawnEndEnabled;
    }
    ...
    public void SharpTimerTopConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        topEnabled = bool.TryParse(args, out var topEnabledValue) ? topEnabledValue : args != "0" && topEnabled;
    }
    ...
    public void SharpTimerHelpConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        helpEnabled = bool.TryParse(args, out var helpEnabledValue) ? helpEnabledValue : args != "0" && helpEnabled;
    }
    ...
    public void SharpTimerRankConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        var args = command.ArgString;
        rankEnabled = bool.TryParse(args, out var rankEnabledValue) ? rankEnabledValue : args != "0" && rankEnabled;
    ...
    public void SharpTimerGoToConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        goToEnabled = bool.TryParse(args, out var goToEnabledValue) ? goToEnabledValue : args != "0" && goToEnabled;
    }
    ...
    public void SharpTimerRemoveLegsConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        removeLegsEnabled = bool.TryParse(args, out var removeLegsEnabledValue)
            ? removeLegsEnabledValue
            : args != "0" && removeLegsEnabled;
    }

    [ConsoleCommand("sharptimer_remove_damage",
        "Whether dealing damage should be disabled or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerRemoveDamageConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        disableDamage = bool.TryParse(args, out var disableDamageValue)
            ? disableDamageValue
            : args != "0" && disableDamage;
    }

    [ConsoleCommand("sharptimer_remove_collision",
        "Whether Player collision should be removed or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerRemoveCollisionConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        removeCollisionEnabled = bool.TryParse(args, out var removeCollisionEnabledValue)
            ? removeCollisionEnabledValue
            : args != "0" && removeCollisionEnabled;
    }

    [ConsoleCommand("sharptimer_checkpoints_enabled",
        "Whether !cp, !tp and !prevcp are enabled by default or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCPConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        cpEnabled = bool.TryParse(args, out var cpEnabledValue) ? cpEnabledValue : args != "0" && cpEnabled;
    }

    [ConsoleCommand("sharptimer_remove_checkpoints_restrictions",
        "Whether checkpoints should save in the air with the current player speed. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCPRestrictConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        removeCpRestrictEnabled = bool.TryParse(args, out var removeCpRestrictEnabledValue)
            ? removeCpRestrictEnabledValue
            : args != "0" && removeCpRestrictEnabled;
    }

    [ConsoleCommand("sharptimer_disable_telehop",
        "Whether the players speed should loose all speed when entring a teleport map trigger or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerResetTeleportSpeedConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        resetTriggerTeleportSpeedEnabled = bool.TryParse(args, out var resetTriggerTeleportSpeedEnabledValue)
            ? resetTriggerTeleportSpeedEnabledValue
            : args != "0" && resetTriggerTeleportSpeedEnabled;
    }

    [ConsoleCommand("sharptimer_max_start_speed_enabled",
        "Whether the players speed should be limited on exiting the starting trigger or not. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMaxStartSpeedBoolConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        maxStartingSpeedEnabled = bool.TryParse(args, out var maxStartingSpeedEnabledValue)
            ? maxStartingSpeedEnabledValue
            : args != "0" && maxStartingSpeedEnabled;
    }

    [ConsoleCommand("sharptimer_max_start_speed",
        "Defines max speed the player is allowed to have while exiting the start trigger. Default value: 320")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerMaxStartSpeedConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var speed) && speed > 0)
        {
    ...
    [ConsoleCommand("sharptimer_max_bonus_start_speed",
        "Defines max speed the player is allowed to have while exiting the start trigger. Default value: 320")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_force_knife_speed",
        "Whether the players speed should be always knife speed regardless of weapon held. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerForceKnifeSpeedBoolConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        forcePlayerSpeedEnabled = bool.TryParse(args, out var forcePlayerSpeedEnabledValue)
            ? forcePlayerSpeedEnabledValue
            : args != "0" && forcePlayerSpeedEnabled;
    }

    [ConsoleCommand("sharptimer_forced_player_speed",
        "Speed override for sharptimer_force_knife_speed. Default value: 250")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    public void SharpTimerBhopBlockTicksConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (int.TryParse(args, out var bhopTicks) && bhopTicks > 0)
        {
    ...
    [ConsoleCommand("sharptimer_stage_times_enabled",
        "Whether stage time records are enabled by default or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerStageTimeConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        enableStageTimes = bool.TryParse(args, out var enableStageTimesValue)
            ? enableStageTimesValue
            : args != "0" && enableStageTimes;
    }

    [ConsoleCommand("sharptimer_connect_commands_msg_enabled",
        "Whether commands on join messages are enabled by default or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerConnectCmdMSGConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        cmdJoinMsgEnabled = bool.TryParse(args, out var cmdJoinMsgEnabledValue)
            ? cmdJoinMsgEnabledValue
            : args != "0" && cmdJoinMsgEnabled;
    }

    [ConsoleCommand("sharptimer_connectmsg_enabled",
        "Whether connect/disconnect messages are enabled by default or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerConnectMSGConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        connectMsgEnabled = bool.TryParse(args, out var connectMsgEnabledValue)
            ? connectMsgEnabledValue
            : args != "0" && connectMsgEnabled;
    }

    [ConsoleCommand("sharptimer_remove_crouch_fatigue",
        "Whether the player should get no crouch fatigue or not. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerRemoveCrouchFatigueConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        removeCrouchFatigueEnabled = bool.TryParse(args, out var removeCrouchFatigueEnabledValue)
            ? removeCrouchFatigueEnabledValue
            : args != "0" && removeCrouchFatigueEnabled;
    }

    [ConsoleCommand("sharptimer_checkpoints_only_when_timer_stopped",
        "Will only allow checkpoints if timer is stopped using !timer")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerCheckpointsOnlyWithStoppedTimer(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        cpOnlyWhenTimerStopped = bool.TryParse(args, out var cpOnlyWhenTimerStoppedValue)
            ? cpOnlyWhenTimerStoppedValue
            : args != "0" && cpOnlyWhenTimerStopped;
    }
    ...
    public void SharpTimerAdServerRecordEnabled(CCSPlayerController? player, CommandInfo command)
    {
        ...
        adServerRecordEnabled = bool.TryParse(args, out var adSREnabledValue)
            ? adSREnabledValue
            : args != "0" && adServerRecordEnabled;
    }

    [ConsoleCommand("sharptimer_ad_sr_timer",
        "Interval how often the messages shall be printed to chat. Default value: 240")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerAdServerRecordTimer(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        if (int.TryParse(args, out var interval) && interval > 0)
        {
    ...
    public void SharpTimerAdMessagesEnabled(CCSPlayerController? player, CommandInfo command)
    {
        ...
        adMessagesEnabled = bool.TryParse(args, out var adCommandsEnabledValue)
            ? adCommandsEnabledValue
            : args != "0" && adMessagesEnabled;
    }

    [ConsoleCommand("sharptimer_ad_messages_timer",
        "Interval how often the messages shall be printed to chat. Default value: 120")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_hud_secondary_color_dynamic",
        "Whether to use dynamic color for secondary HUD based on player speed. Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerHUDSecondaryColorDynamicConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        useDynamicColor = bool.TryParse(args, out var useDynamicColorValue)
            ? useDynamicColorValue
            : args != "0" && useDynamicColor;
    }
    ...
    public void SharpTimerFakeTriggerHeightConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (float.TryParse(args, out var height) && height > 0)
        {
    ...
    public void SharpTimerZones3DBox(CCSPlayerController? player, CommandInfo command)
    {
        ...
        Box3DZones = bool.TryParse(args, out var ZoneBoxEnabledValue) ? ZoneBoxEnabledValue : args != "0" && Box3DZones;
    }
    ...
    public void SharpTimerStartzoneJumpConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        startzoneJumping = bool.TryParse(args, out var startzoneJumpingValue)
            ? startzoneJumpingValue
            : args != "0" && startzoneJumping;
    }
    ...
    public void SharpTimerSpawnOnRespawnPos(CCSPlayerController? player, CommandInfo command)
    {
        ...
        spawnOnRespawnPos = bool.TryParse(args, out var spawnOnRespawnPosValue)
            ? spawnOnRespawnPosValue
            : args != "0" && spawnOnRespawnPos;
    }

    /* sounds convars */
    [ConsoleCommand("sharptimer_enable_sounds_by_default",
        "Whether to enable sounds for players by default.Default value: false")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerSoundEnableByDefault(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        soundsEnabledByDefault = bool.TryParse(args, out var soundsEnabledByDefaultValue)
            ? soundsEnabledByDefaultValue
            : args != "0" && soundsEnabledByDefault;
    }
    ...
    [ConsoleCommand("sharptimer_sound_checkpoint",
        "Defines Checkpoint sound. Default value: sounds/ui/buttonclick.vsnd")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_sound_checkpoint_error",
        "Defines Checkpoint Error sound. Default value: sounds/ui/weapon_cant_buy.vsnd")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    [ConsoleCommand("sharptimer_sound_sr",
        "Defines SR Sound. Default value: sounds/ui/panorama/round_report_round_won_01.vsnd")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    ...
    public void SharpTimerSoundServerRecordAllPlayers(CCSPlayerController? player, CommandInfo command)
    {
        ...
        srSoundAll = bool.TryParse(args, out var soundSRAllValue) ? soundSRAllValue : args != "0" && srSoundAll;
    }

    [ConsoleCommand("sharptimer_sound_stage_all_players",
        "Whether to play stage record sound for all players. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerSoundStageRecordAllPlayers(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        stageSoundAll = bool.TryParse(args, out var stageSoundAllValue)
            ? stageSoundAllValue
            : args != "0" && stageSoundAll;
    }
    /* sounds convars */
    ...
    public void SharpTimerEnableNoclipConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        enableNoclip = bool.TryParse(args, out var enableNoclipValue) ? enableNoclipValue : args != "0" && enableNoclip;
    }
    ...
    public void SharpTimerEnableStylesConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        enableStyles = bool.TryParse(args, out var enableStylesValue) ? enableStylesValue : args != "0" && enableStyles;
    }

    [ConsoleCommand("sharptimer_style_points_enabled",
        "Enable or disable points granted for style runs. Default value: true")]
    [CommandHelper(whoCanExecute: CommandUsage.SERVER_ONLY)]
    public void SharpTimerEnableStylePointsConvar(CCSPlayerController? player, CommandInfo command)
    {
        var args = command.ArgString;

        enableStylePoints = bool.TryParse(args, out var enableStylePointsValue)
            ? enableStylePointsValue
            : args != "0" && enableStylePoints;
    }
    ...
    public void SharpTimerLowGravMultiplierConvar(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (double.TryParse(args, out var pointModifier) && pointModifier > 0)
        {
    ...
    public void SharpTimerRemoteDataOverrideBhop(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (string.IsNullOrEmpty(args))
        {
            remoteBhopDataSource =
                "https://raw.githubusercontent.com/Letaryat/poor-SharpTimer/main/remote_data/bhop_.json";
            return;
        }
    ...
    public void SharpTimerRemoteDataOverrideSurf(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (string.IsNullOrEmpty(args))
        {
            remoteSurfDataSource =
                "https://raw.githubusercontent.com/Letaryat/poor-SharpTimer/main/remote_data/surf_.json";
            return;
        }
