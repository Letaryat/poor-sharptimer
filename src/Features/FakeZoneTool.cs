...
public partial class SharpTimer
{
    ...
    public void FakeZonesMenu(CCSPlayerController? player, CommandInfo command)
    {
        ...
        zonesmenu.AddMenuOption("startzone", (player, option) => { AddStartZoneCommand(player); });
        zonesmenu.AddMenuOption("endzone", (player, option) => { AddEndZoneCommand(player); });
        zonesmenu.AddMenuOption("respawnpos", (player, option) => { AddRespawnPosCommand(player); });
        zonesmenu.AddMenuOption("save", (player, option) => { SaveZonesCommand(player); });
    ...
    public void AddStartZoneCommand(CCSPlayerController? player)
    {
        ...
        if (playerTimers[player!.Slot].IsAddingStartZone)
        {
            ...
            playerTimers[player.Slot].StartZoneC2 =
                $"{player.Pawn.Value!.CBodyComponent?.SceneNode?.AbsOrigin.X} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Y} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Z}";
        ...
        else
        {
            ...
            playerTimers[player.Slot].IsAddingEndZone = false;
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Please go to the opposite zone corner now");
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}and type {primaryChatColor}!1 {ChatColors.Default}again");
        }
    }
    ...
    public void AddEndZoneCommand(CCSPlayerController? player)
    {
        ...
        if (playerTimers[player!.Slot].IsAddingEndZone)
        {
            playerTimers[player.Slot].IsAddingStartZone = false;
            playerTimers[player.Slot].IsAddingEndZone = false;
        ...
        else
        {
            ...
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Please go to the opposite zone corner now");
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}and type {primaryChatColor}!2 {ChatColors.Default}again");
        }
    }
    ...
    public void SaveZonesCommand(CCSPlayerController? player)
    {
        ...
        if (playerTimers[player!.Slot].EndZoneC1 == null || playerTimers[player.Slot].EndZoneC2 == null ||
            playerTimers[player.Slot].StartZoneC1 == null || playerTimers[player.Slot].StartZoneC2 == null ||
            playerTimers[player.Slot].RespawnPos == null)
        {
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Red}Please make sure you have done all 3 zoning steps (startzone, endzone, respawnpos)");
            return;
        }
        ...
        var mapdataFileName = $"SharpTimer/MapData/{currentMapName}.json"; // Use the map name in the filename
        var mapdataPath = Path.Join(gameDir + "/csgo/cfg", mapdataFileName);

        var updatedJson = JsonSerializer.Serialize(newMapInfo, jsonSerializerOptions);
        File.WriteAllText(mapdataPath, updatedJson);

        player.PrintToChat(
            $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Zones saved successfully! {ChatColors.Grey}Reloading data...");
    ...
    public void AddBonusStartZoneCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (playerTimers[player!.Slot].IsAddingBonusStartZone)
        {
        ...
        {
            playerTimers[player.Slot].BonusStartZoneC1 =
                $"{player.Pawn.Value!.CBodyComponent?.SceneNode?.AbsOrigin.X} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Y} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Z}";
            playerTimers[player.Slot].BonusStartZoneC2 = "";
            ...
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Please go to the opposite zone corner now");
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}and type {primaryChatColor}!addbonusstartzone {ChatColors.Default}again");
        }
    }
    ...
    public void AddBonusEndZoneCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (playerTimers[player!.Slot].IsAddingBonusEndZone)
        {
            playerTimers[player.Slot].IsAddingBonusStartZone = false;
            playerTimers[player.Slot].IsAddingBonusEndZone = false;
        ...
        {
            playerTimers[player.Slot].BonusEndZoneC1 =
                $"{player.Pawn.Value!.CBodyComponent?.SceneNode?.AbsOrigin.X} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Y} {player.Pawn.Value.CBodyComponent?.SceneNode?.AbsOrigin.Z}";
            playerTimers[player.Slot].BonusEndZoneC2 = "";
            playerTimers[player.Slot].IsAddingBonusStartZone = false;
            ...
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Please go to the opposite zone corner now");
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}and type {primaryChatColor}!addbonusendzone {ChatColors.Default}again");
        }
    }
    ...
    public void SaveBonusZonesCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (playerTimers[player!.Slot].BonusEndZoneC1 == null || playerTimers[player.Slot].BonusEndZoneC2 == null ||
            playerTimers[player.Slot].BonusStartZoneC1 == null || playerTimers[player.Slot].BonusStartZoneC2 == null ||
            playerTimers[player.Slot].BonusRespawnPos == null)
        {
            player.PrintToChat(
                $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Red}Please make sure you have done all 3 zoning steps (!addbonusstartzone, !addbonusendzone, !addbonusrespawnpos)");
            return;
        }
        ...
        if (!int.TryParse(command.ArgString, out var bonusX))
        {
            ...
            player.PrintToChat(
                $" {Localizer["prefix"]} Please enter a valid Bonus stage i.e: {primaryChatColor}!savebonuszones <index>");
            return;
        }
        ...
        var mapdataFileName =
            $"SharpTimer/MapData/{currentMapName}_bonus{bonusX}.json"; // Use the map name in the filename
        var mapdataPath = Path.Join(gameDir + "/csgo/cfg", mapdataFileName);
        ...
        player.PrintToChat(
            $" {ChatColors.LightPurple}[ZONE TOOL] {ChatColors.Default}Bonus {bonusX} Zones saved successfully! {ChatColors.Grey}Reloading data...");
        Server.ExecuteCommand("mp_restartgame 1");
    }
    ...
    public void OnTickZoneTool(CCSPlayerController? player)
    {
        try
        {
            ...
            if (playerTimers.TryGetValue(player.Slot, out var playerTimer))
            {
                ...
                {
                    var pawnPosition = player.Pawn?.Value!.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0);
    ...
    public void DrawZoneToolWire(Vector startPos, Vector endPos, int playerSlot, int wireIndex)
    {
        try
        {
            if (playerTimers.ContainsKey(playerSlot) && playerTimers[playerSlot] != null)
            {
                ...
                var wire = playerTimers[playerSlot].ZoneToolWire![wireIndex];
