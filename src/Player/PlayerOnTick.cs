...
public partial class SharpTimer
{
        ...
        try
        {
            ...
            foreach (var player in connectedPlayers.Values)
            {
                ...
                if (playerTimers[playerSlot].IsAddingStartZone || playerTimers[playerSlot].IsAddingEndZone ||
                    playerTimers[playerSlot].IsAddingBonusStartZone || playerTimers[playerSlot].IsAddingBonusEndZone)
                {
                    OnTickZoneTool(player);
                    continue;
                }

                if (playerTimers.TryGetValue(playerSlot, out var playerTimer) && IsAllowedPlayer(player))
                {
                    ...
                    var isOnBhopBlock = playerTimer.IsOnBhopBlock;
                    var isTimerRunning = playerTimer.IsTimerRunning;
                    var isBonusTimerRunning = playerTimer.IsBonusTimerRunning;
                    var isTimerBlocked = playerTimer.IsTimerBlocked;
                    var timerTicks = playerTimer.TimerTicks;
                    ...
                    if (playerTimer.MovementService!.OldJumpPressed) playerTimer.MovementService.OldJumpPressed = false;

                    if (playerTimer.currentStyle.Equals(4)) //check if 400vel
                        SetVelocity(player, player!.Pawn.Value!.AbsVelocity, 400);

                    if (playerTimer.currentStyle.Equals(10) &&
                        !player.PlayerPawn.Value.GroundEntity.IsValid) //check if ff
                        if (currentTick % 2 != 0)
                    ...
                    if (isOnBhopBlock)
                    {
                        ...
                        if (playerTimer.TicksOnBhopBlock > bhopBlockTime) RespawnPlayer(player);
                    }

                    if (useTriggers == false && isTimerBlocked == false) CheckPlayerCoords(player, playerSpeed);
                    if (useTriggers && isTimerBlocked == false && useTriggersAndFakeZones)
                        CheckPlayerCoords(player, playerSpeed);

                    if (jumpStatsEnabled)
                        OnJumpStatTick(player, playerSpeed, player.Pawn?.Value!.CBodyComponent?.SceneNode!.AbsOrigin!,
                            player.PlayerPawn?.Value.EyeAngles!, playerButtons);
                    if (StrafeHudEnabled) OnSyncTick(player, playerButtons, player.PlayerPawn?.Value.EyeAngles!);
                    if (StrafeHudEnabled && playerTimers[player.Slot].inStartzone && playerTimer.Rotation.Count > 0)
                    {
                    ...
                    {
                        var designerName = player.Pawn!.Value!.WeaponServices!.ActiveWeapon?.Value?.DesignerName ??
                                           "no_knife";
                    ...
                    if (playerTimer.CachedMapPlacement == null && !playerTimer.IsRankPbReallyCached)
                    {
                        ...
                        var steamID = player.SteamID.ToString();
                        SharpTimerDebug(
                            $"{playerName} CachedMapPlacement is still null, calling rank handler once more");
                        AddTimer(3.0f,
                            () =>
                            {
                                _ = Task.Run(async () => await RankCommandHandler(player, steamID, playerSlot,
                                    playerName, true, playerTimer.currentStyle));
                            });
                    ...
                    if (playerTimer.IsSpecTargetCached == false ||
                        specTargets.ContainsKey(player.Pawn!.Value!.EntityHandle.Index) == false)
                    {
                    ...
                    if (removeCollisionEnabled)
                        if (player.PlayerPawn!.Value.Collision.CollisionGroup !=
                            (byte)CollisionGroup.COLLISION_GROUP_DISSOLVING ||
                            player.PlayerPawn.Value.Collision.CollisionAttribute.CollisionGroup !=
                            (byte)CollisionGroup.COLLISION_GROUP_DISSOLVING)
                        {
                    ...
                    if (enableReplays)
                    {
                        if (!playerTimer.IsReplaying && (timerTicks > 0 || playerTimer.BonusTimerTicks > 0) &&
                            playerTimer.IsRecordingReplay && !isTimerBlocked) ReplayUpdate(player, timerTicks);
                        ...
                        {
                            if (!isTimerBlocked &&
                                (player.PlayerPawn!.Value.MoveType.HasFlag(MoveType_t.MOVETYPE_OBSERVER) ||
                                 player.PlayerPawn.Value.ActualMoveType.HasFlag(MoveType_t.MOVETYPE_OBSERVER)))
                    ...
                    var keyEnabled = !playerTimer.HideKeys && keysOverlayEnabled;
                    var hudEnabled = !playerTimer.HideTimerHud && hudOverlayEnabled;

                    var formattedPlayerVel = Math.Round(use2DSpeed
                            ? playerSpeed.Length2D()
                            : playerSpeed.Length())
                        .ToString("0000");
                    var playerVel = int.Parse(formattedPlayerVel);

                    var secondaryHUDcolorDynamic = "LimeGreen";
                    int[] velocityThresholds = { 349, 699, 1049, 1399, 1749, 2099, 2449, 2799, 3149, 3499 };
                    string[] hudColors =
                    {
                        "LimeGreen", "Lime", "GreenYellow", "Yellow", "Gold", "Orange", "DarkOrange", "Tomato",
                        "OrangeRed", "Red", "Crimson"
                    };

                    for (var i = 0; i < velocityThresholds.Length; i++)
                    ...
                    var playerVelColor = useDynamicColor ? secondaryHUDcolorDynamic : secondaryHUDcolor;
                    var formattedPlayerPre = Math.Round(ParseVector(playerTimer.PreSpeed ?? "0 0 0").Length2D())
                        .ToString("000");
                    var playerTime = FormatTime(timerTicks);
                    var playerBonusTime = FormatTime(playerTimer.BonusTimerTicks);
                    var timerLine = isBonusTimerRunning
                        ...
                        : isTimerRunning
                            ? $" <font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Timer: </font><font class='fontSize-l horizontal-center' color='{primaryHUDcolor}'>{playerTime}</font> <font color='gray' class='fontSize-s stratum-bold-italic'>({GetPlayerPlacement(player)})</font>{(playerTimer.CurrentMapStage != 0 && useStageTriggers ? $" <font color='gray' class='fontSize-s stratum-bold-italic'> {playerTimer.CurrentMapStage}/{stageTriggerCount}</font>" : "")} <br>"
                    ...
                    var veloLine =
                        $" {(playerTimer.IsTester ? playerTimer.TesterSmolGif : "")}<font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Speed:</font> {(playerTimer.IsReplaying ? "<font class=''" : "<font class='fontSize-l horizontal-center'")} color='{playerVelColor}'>{formattedPlayerVel}</font> <font class='fontSize-s stratum-bold-italic' color='gray'>({formattedPlayerPre})</font>{(playerTimer.IsTester ? playerTimer.TesterSmolGif : "")} <br>";

                    var syncLine =
                        $"<font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Sync:</font> <font class='fontSize-l horizontal-center color='{secondaryHUDcolor}'>{playerTimer.Sync:F2}%</font> <br>"; //2f
                    ...
                    var keysLineNoHtml =
                        $"{(hudEnabled ? "<br>" : "")}<font class='fontSize-ml stratum-light-mono' color='{tertiaryHUDcolor}'>{((playerButtons & PlayerButtons.Moveleft) != 0 ? "A" : "_")} " +
                    ...
                    var hudContent = (hudEnabled
                                     ...
                                     (playerTimer.IsTester && !playerTimer.IsReplaying
                                         ? $"{(!keyEnabled ? "<br>" : "")}" + playerTimer.TesterBigGif
                                         : "") +
                                     (playerTimer.IsVip && !playerTimer.IsTester && !playerTimer.IsReplaying
                                         ? $"{(!keyEnabled ? "<br><br>" : "")}" +
                                           $"<br><img src='https://files.catbox.moe/{playerTimer.VipBigGif}.gif'><br>"
                                         : "") +
                                     (playerTimer.IsReplaying && playerTimer.VipReplayGif != "x"
                                         ? playerTimer.VipReplayGif
                                         : "");

                    if (hudEnabled || keyEnabled) player.PrintToCenterHtml(hudContent);
                }
            }
        }
        catch (Exception ex)
    ...
    public void SpectatorOnTick(CCSPlayerController player)
    {
        ...
        try
        {
            ...
            if (playerTimers.TryGetValue(target.Slot, out var playerTimer) && IsAllowedPlayer(target))
            {
                var isTimerRunning = playerTimer.IsTimerRunning;
                var isBonusTimerRunning = playerTimer.IsBonusTimerRunning;
                var timerTicks = playerTimer.TimerTicks;
                ...
                var keyEnabled = !playerTimer.HideKeys && !playerTimer.IsReplaying && keysOverlayEnabled;
                var hudEnabled = !playerTimer.HideTimerHud && hudOverlayEnabled;
                ...
                var formattedPlayerVel = Math.Round(use2DSpeed
                        ...
                        : playerSpeed.Length())
                    .ToString("0000");
                var formattedPlayerPre =
                    Math.Round(ParseVector(playerTimer.PreSpeed ?? "0 0 0").Length2D()).ToString("000");
                var playerTime = FormatTime(timerTicks);
                var playerBonusTime = FormatTime(playerTimer.BonusTimerTicks);
                var timerLine = isBonusTimerRunning
                    ? $" <font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Bonus #{playerTimer.BonusStage} Timer:</font> <font class='fontSize-l horizontal-center' color='{primaryHUDcolor}'>{playerBonusTime}</font> <br>"
                    : isTimerRunning
                        ? $" <font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Timer: </font><font class='fontSize-l horizontal-center' color='{primaryHUDcolor}'>{playerTime}</font> <font color='gray' class='fontSize-s stratum-bold-italic'>({GetPlayerPlacement(target)})</font>{(playerTimer.CurrentMapStage != 0 && useStageTriggers ? $" <font color='gray' class='fontSize-s stratum-bold-italic'> {playerTimer.CurrentMapStage}/{stageTriggerCount}</font>" : "")} <br>"
                        : playerTimer.IsReplaying
                            ? $" <font class='horizontal-center' color='red'>◉ REPLAY {FormatTime(playerReplays[target.Slot].CurrentPlaybackFrame)}</font> <br>"
                            : "";

                var veloLine =
                    $" {(playerTimer.IsTester ? playerTimer.TesterSmolGif : "")}<font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'>Speed:</font> {(playerTimer.IsReplaying ? "<font class=''" : "<font class='fontSize-l horizontal-center'")} color='{secondaryHUDcolor}'>{formattedPlayerVel}</font><font class='fontSize-s stratum-bold-italic' color='{tertiaryHUDcolor}'> u/s</font> <font class='fontSize-s stratum-bold-italic' color='gray'>({formattedPlayerPre} u/s)</font>{(playerTimer.IsTester ? playerTimer.TesterSmolGif : "")} <br>";
                ...
                var keysLineNoHtml =
                    $"{(hudEnabled ? "<br>" : "")}<font class='fontSize-ml stratum-bold-mono' color='{tertiaryHUDcolor}'>{((playerButtons & PlayerButtons.Moveleft) != 0 ? "A" : "_")} " +
                    $"{((playerButtons & PlayerButtons.Forward) != 0 ? "W" : "_")} " +
                    $"{((playerButtons & PlayerButtons.Moveright) != 0 ? "D" : "_")} " +
                    $"{((playerButtons & PlayerButtons.Back) != 0 ? "S" : "_")} " +
                    $"{((playerButtons & PlayerButtons.Jump) != 0 || playerTimer.MovementService!.OldJumpPressed ? "J" : "_")} " +
