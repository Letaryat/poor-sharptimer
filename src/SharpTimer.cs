...
{
    private static readonly MemoryFunctionVoid<CCSPlayerPawn, CSPlayerState> StateTransition =
        new(GameData.GetSignature("StateTransition"));
    ...
    public override void Load(bool hotReload)
    {
        ...
        {
            foreach (var player in connectedPlayers.Values) InvalidateTimer(player);
            return HookResult.Continue;
        }, HookMode.Pre);
        ...
        RegisterEventHandler<EventPlayerSpawn>((@event, info) =>
        {
            ...
            if (player.IsValid && !player.IsBot) OnPlayerSpawn(player);
        ...
        RegisterEventHandler<EventPlayerJump>((@event, info) =>
        {
            if (@event.Userid!.IsValid)
            {
                ...
                if (jumpStatsEnabled) OnJumpStatJumped(player);
            }
        ...
        RegisterEventHandler<EventPlayerSound>((@event, info) =>
        {
            if (@event.Userid!.IsValid)
            {
                ...
                if (jumpStatsEnabled && @event.Step) OnJumpStatSound(player);
            }
        ...
        HookEntityOutput("trigger_multiple", "OnStartTouch", (output, name, activator, caller, value, delay) =>
        {
            TriggerMultipleOnStartTouch(activator, caller);
            return HookResult.Continue;
        });

        HookEntityOutput("trigger_multiple", "OnEndTouch", (output, name, activator, caller, value, delay) =>
        {
            TriggerMultipleOnEndTouch(activator, caller);
            return HookResult.Continue;
        });

        HookEntityOutput("trigger_teleport", "OnEndTouch", (output, name, activator, caller, value, delay) =>
        {
            TriggerTeleportOnEndTouch(activator, caller);
            return HookResult.Continue;
        });

        HookEntityOutput("trigger_teleport", "OnStartTouch", (output, name, activator, caller, value, delay) =>
        {
    ...
    private void ForceFullUpdate(CCSPlayerController? player)
    {
        ...
        player.PlayerPawn.Value?.Teleport(null, player.PlayerPawn.Value.EyeAngles);
    }
